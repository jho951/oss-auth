...
class DefaultSamlResponseVerifierTest {
	...
	void verifiesSignatureAndExtractsNormalizedAssertion() {
		...
		SamlAssertion assertion = verifier.verify(new SamlAuthenticationRequest(sampleResponseXml(), "", "", "", java.util.Map.of()));
		...
		assertThat(assertion.getAttributes().get("groups")).isEqualTo(List.of("admin", "ops"));
	}
